...
public interface ToolMath {
     ...
     * @return 计算结果
     */
    static double doubleDivNoKeep(double d1, double d2, @IntRange(from = 0) int scale) {
        if (d2 == 0) {
            return 0d;
        }
        BigDecimal bd1 = BigDecimal.valueOf(d1);
        BigDecimal bd2 = BigDecimal.valueOf(d2);
        return bd1.divide(bd2, scale, BigDecimal.ROUND_HALF_DOWN).doubleValue();
    }
     ...
     * @return 计算结果
     */
    static double doubleDivNoEnter(double d1, double d2, @IntRange(from = 0) int scale) {
        if (d2 == 0) {
            return 0d;
        }
        BigDecimal bd1 = BigDecimal.valueOf(d1);
     ...
     * @param d 数字
     */
    static String removePointZero(double d) {
